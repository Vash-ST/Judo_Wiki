...
const galleryData: Record<string, {jp: string, es: string, videoUrl: string}[]> = {
  'Pie/Pierna': [
...
export default function CursoDetalle() {
  ...
  const [activeMenu, setActiveMenu] = useState('Pie/Pierna');
  const [selectedVideo, setSelectedVideo] = useState<{jp: string, es: string, videoUrl: string} | null>(null);
    ...
    <div className="min-h-screen bg-[#D62828] p-8 md:p-16 text-white">
      {selectedVideo && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setSelectedVideo(null)}>
          <div className="bg-white rounded-lg max-w-4xl w-full h-[80vh] text-black overflow-hidden flex items-center justify-center" onClick={e => e.stopPropagation()}>
            {selectedVideo.videoUrl ? (
              ...
              <div className="text-center p-8">
                <h2 className="text-2xl font-bold mb-4">{selectedVideo.jp}</h2>
                <p>Video próximamente disponible.</p>
                <button className="mt-6 bg-[#2a4d69] text-white px-6 py-2 rounded" onClick={() => setSelectedVideo(null)}>Cerrar</button>
              </div>
            )}
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {activeModule === 'TÉCNICAS' && (
            ...
            >
              ...
              {/* Gallery Section with circular menu and connecting lines */}
              <div className="mt-12 flex flex-col items-center">
                <div className="flex flex-wrap justify-center gap-8 relative items-center">
                  {/* Decorative connecting lines */}
                  <div className="absolute top-1/2 left-0 w-full h-0.5 bg-gray-300 -z-0"></div>
                  
                  {['Pie/Pierna', 'Suelo', 'Escapes', 'Cadera', 'Mano/Brazo'].map((item) => (
                    <button 
                      key={item} 
                      onClick={() => setActiveMenu(item)}
                      className={`relative z-10 w-24 h-24 rounded-full border-4 flex items-center justify-center text-center font-bold text-xs transition-transform hover:scale-110 ${activeMenu === item ? 'bg-[#2a4d69] text-white border-[#2a4d69]' : 'bg-white border-[#2a4d69] text-[#2a4d69]'}`}>
                      {item}
                    </button>
                  ))}
                </div>
                ...
                <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-6">
                  {galleryData[activeMenu].map((item, idx) => (
                    <motion.div
                      ...
                      className="p-6 bg-[#2a4d69] text-white flex flex-col items-center justify-center text-center shadow-lg cursor-pointer"
                      style={{
                    ...
                    >
                      <span className="font-bold text-lg mb-1">{item.jp}</span>
                      <span className="text-sm opacity-90 leading-tight">{item.es}</span>
                    </motion.div>
                  ))}
                </div>
              </div>
          ...
          {activeModule === 'TEST' && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.3 }}
              className="mt-16 bg-white p-8 border-t-4 border-black text-black shadow-xl"
            >
              <h3 className="text-2xl font-bold mb-6 text-black uppercase tracking-widest text-center">Test de Conocimientos</h3>
              <div className="text-center p-8 bg-gray-50 border border-gray-200">
                <p className="text-lg mb-6">Pon a prueba todo lo que has aprendido en las secciones de Teoría y Técnicas.</p>
                <button className="px-8 py-3 bg-black text-white font-bold tracking-wider hover:bg-gray-800 transition-colors uppercase">
                  Comenzar Test
                </button>
              </div>
            </motion.div>
